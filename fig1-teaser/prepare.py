...
config = {
    "mesh": sorted(list(glob.glob("./meshes/*.obj"))),
    "problem": "Cubic",
    "normalize_mesh": True,
    "discr_order": 1,
...
config_ref = {
    "mesh": sorted(list(glob.glob("./meshes/*.obj")))[0],
    "problem": "Cubic",
    "normalize_mesh": True,
    "discr_order": 4,
    "use_p_ref": False,
...
def generate_jobs(entry):
    ...
    all_vals = itertools.product(*[v for __, v in entry.items() if isinstance(v, list)])
    all_jobs = []
...
def write_job(entry, job_id, output_dir, job_dir):
    ...
    entry["export"]["iso_mesh"] = os.path.relpath(baseout + '_iso.obj', job_dir)
